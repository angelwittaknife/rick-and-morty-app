...
class PlaceDao extends DatabaseAccessor<AppDatabase> with _$PlaceDaoMixin {
  ...
  Future<Place> getPlaceById(int id) =>
      (select(places)..where((tbl) => tbl.id.equals(id))).getSingle();

  Future<List<Place>> getPlacesByIds(List<int> ids) =>
    (select(places)..where((tbl) => tbl.id.isIn(ids))).get();
  ...
  Future<void> deletePlace(Place place) =>
      (delete(places)..where((tbl) => tbl.name.equals(place.name))).go();
